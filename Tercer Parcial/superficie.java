...
public class superficie extends JFrame implements KeyListener{
    ...
    private final int puntosCurva = 200;
    private int profundidad = 10;

    int space[][][] = new int [profundidad][puntosCurva][4];
    int curva[][] = new int[puntosCurva][4];
    double[] repintarRotacion = {0,-Math.PI/4,Math.PI*2/16};
    int[] repintarTraslacion = {0, 0, 0};
    int[] traslacionCurva = {200, 200, 0};
    int cuboTransformacion[][] = new int[8][4];
    double[] repintarEscalacion = {1,1,1};
    ...
    public void formulaCurva(int curva[][], int x, double expX, int z){
        double coorX, coorY;

        double maxX = x, minX = x;
        double maxY = 0, minY = 0;
        double maxZ = z, minZ = z;
        
        for (int i = 0; i < puntosCurva; i++) {

            coorX = (double)x + (expX * i);
            coorY = Math.sin(coorX) + 1;   //sen(x) + 1
        ...
        for (int i = 0; i < profundidad; i++) {
            for (int j = 0; j < puntosCurva; j++){
                space[i][j][0] = curva[j][0];
                space[i][j][1] = curva[j][1] + rM/2;
                space[i][j][2] = curva[j][2] + i*rM/profundidad;
                space[i][j][3] = curva[j][3];
            }
        }
    }

    public void curve(int curva[][], int xp, int yp, int zp){
        int curva2D[][] = new int [puntosCurva][2];

        for (int i = 0; i < puntosCurva; i++) {
            curva2D[i] = calcular2D(curva[i][0], curva[i][1], curva[i][2], xp,  yp, zp); 
        }

        for (int i = 1; i < puntosCurva; i++) {
            puntoMedio(curva2D[i-1][0], curva2D[i-1][1], curva2D[i][0], curva2D[i][1]);
        }
    }

    public void drawSup(int space[][][], int xp, int yp, int zp){
        int space2D[][][] = new int [profundidad][puntosCurva][3];
